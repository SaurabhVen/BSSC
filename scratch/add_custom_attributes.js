...
async function run() {
  ...
  console.log(`Starting to add custom attributes to User Pool: ${COGNITO_USER_POOL_ID} in region ${AWS_REGION}`);
  ...
  try {
    ...
    console.log('Successfully added custom attributes to Cognito User Pool!', JSON.stringify(response, null, 2));
  } catch (err) {
    console.error('Failed to add custom attributes:', err.message);
    if (err.stack) {
