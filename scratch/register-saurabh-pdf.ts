...
async function run() {
  ...
  try {
    ...
    // 3. Register user via Cognito SDK SignUpCommand
    console.log('Calling Cognito SignUp command with attributes from the PDF...');
    ...
    const cognitoAttrs = [
      ...
      { Name: 'custom:bihar_domicile', Value: 'YES' }, // From PDF
      { Name: 'custom:category', Value: 'BC' }, // From PDF
      { Name: 'custom:caste', Value: '' }, // `--Select--` in PDF
      { Name: 'custom:non_creamy_layer', Value: 'YES' }, // From PDF
      { Name: 'custom:is_pwd', Value: 'NO' }, // From PDF
      { Name: 'custom:disability_type', Value: 'NO' },
      { Name: 'custom:pwd_40_percent', Value: 'NO' }, // From PDF
      { Name: 'custom:ex_serviceman', Value: 'NO' }, // From PDF
      { Name: 'custom:service_period', Value: '0-0-0' },
      { Name: 'custom:ncc_cadet', Value: 'NO' }, // From PDF
      { Name: 'custom:ncc_cert_no', Value: 'NA' },
      { Name: 'custom:bihar_govt_emp', Value: 'NO' }, // From PDF
      { Name: 'custom:bssc_attempts', Value: '0' }, // `--Select--` in PDF
      { Name: 'custom:contractual_emp', Value: 'NO' }, // From PDF
      { Name: 'custom:post_name', Value: '' }, // `--Select--` in PDF
      { Name: 'custom:has_agreement', Value: 'NO' }, // From PDF
